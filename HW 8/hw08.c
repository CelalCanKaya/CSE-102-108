...
void writeRecords(Records records, char* filename) {
	...
	if(ptr==NULL){
		...
		fwrite(&(records.data), sizeof(Records) , 1, ptr);
	}
	fclose(ptr);
}
...
void readRecords(Records *records, char* filename) {
	...
	if(ptr==NULL){
		...
		while(!feof(ptr)){
			fread(&(records->data), sizeof(Records), 1, ptr);
		}
	}
	fclose(ptr);
}
...
void writePeople(People people, char* filename) {
	...
	if(ptr==NULL){
		...
		fwrite(&(people.data), sizeof(People) , 1, ptr);
	}
	fclose(ptr);
}
/*
...
void readPeople(People *people, char* filename) {
	...
	if(ptr==NULL){
		...
		while(!feof(ptr)){
			fread(&(people->data), sizeof(People), 1, ptr);
		}
	}
	fclose(ptr);
}
...
void read(char* filename, People *people, Records *records){
	...
	if(ptr==NULL){
		...
		while(i<lenght("input.txt")){
			fscanf(ptr, "%d %s %s %d", &id, name,surname, &total_num);
			strcat(name, " ");
			...
			for(counter=0; counter<total_num; counter++){
				...
				strcpy(records->data[records->size].number, number);
				records->size ++;
			}
			i++;
		}
